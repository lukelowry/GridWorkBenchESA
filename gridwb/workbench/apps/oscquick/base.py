...
class BaseOscMatrixQ:

    def __init__(self, Y, oscmodels, buses: list[Bus], loads: list[Load]):

        # [[G, -B], [B, G]] Format
        YC = ybus_with_loads(Y, buses, loads)

        # Osc Mastrix - Start with Y Bus in upper left
        self.master = lil_matrix(YC.copy(), dtype=complex)
    ...
    def add_model(self, oscmodel):
        ...
        # Insert Algebraic
        self.master[busidx, end:end+N]  = kcl

        # Insert Algebraic
        self.master[end:end+N, busidx]  = kvl
